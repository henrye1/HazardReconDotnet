-- The transaction number reaches the stored detail rows, so a flagged receivables
-- row can be identified: an account holds many transactions, and without this the
-- untraced table shows several rows that differ only by amount.
--
-- Only run_set_untraced_rows gets the column. The write-off exception rows come
-- from the write-off file, which carries no transaction number and is matched on
-- the account alone, so a column there could only ever be blank.
--
...
alter table public.run_set_untraced_rows
  add column if not exists transaction_number text not null default '';

-- save_run_completion is redefined in full rather than patched, because there is
-- no way to alter one statement of a function body. The signature is unchanged,
-- so this replaces the body in place and keeps the existing grant - adding a
-- parameter would instead create an overload, and PostgREST would then answer
-- /rpc/save_run_completion with 300 Multiple Choices. The grant is restated
-- anyway so this file also works on a database that never had the function.
...
begin
  ...
  loop
    ...
    -- coalesce, because a lending payload has no transaction_number key at all and
    -- ->> would then yield NULL into a not-null column
    insert into public.run_set_untraced_rows
      (run_set_result_id, account, transaction_number, cohort_date, rating, amount, position)
    select v_new_id, c->>'account', coalesce(c->>'transaction_number', ''),
           c->>'cohort_date', c->>'rating', (c->>'amount')::numeric, (c->>'position')::int
